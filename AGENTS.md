...
**ShadcnSpace Dashboard Pro** is a modern Next.js admin dashboard template built with TypeScript, Tailwind CSS, and shadcn/ui components. It features:
...
- `src/app/` — Next.js app directory (single monolith)
- `src/app/(site)/` — Public marketplace pages (header/footer layout)
- `src/app/(dashboard)/` — Dashboard route group
- `src/app/(dashboard)/dashboard/` — Main dashboard layout wrapper (URLs: `/dashboard/*`)
- `src/app/(dashboard)/dashboard/page.tsx` — Dashboard home page
- `src/app/layout.tsx` — Root HTML shell
- `src/app/(dashboard)/layout.tsx` — Dashboard theme and customizer providers

### Application Modules

- `src/app/(dashboard)/dashboard/apps/` — Independent application modules:
...
- `src/app/(dashboard)/dashboard/dashboards/` — Dashboard variants (analytics, ecommerce, etc.)
- `src/app/(dashboard)/dashboard/charts/` — Chart showcase and examples
- `src/app/(dashboard)/dashboard/forms/` — Form patterns and components
- `src/app/(dashboard)/dashboard/react-tables/` — TanStack React Table examples
- `src/app/(dashboard)/dashboard/shadcn-tables/` — shadcn/ui table components
- `src/app/(dashboard)/dashboard/shadcn-ui/` — Complete shadcn/ui component showcase
- `src/app/(dashboard)/dashboard/layout/` — Layout components and patterns
- `src/app/(dashboard)/dashboard/icons/` — Icon library showcase

### Authentication & Security

- `src/app/(dashboard)/auth/` — Authentication pages and flows:
...
- `src/components/Themeprovider.tsx` — Root theme provider
- `src/components/ui/` — shadcn/ui primitive components (button, card, dialog, etc.)
...
- `src/components/dashboard/` — Application-specific components:
...
- `src/app/context/config.ts` — Global configuration (theme, language, layout settings)
- `src/app/context/*/` — Feature-specific context providers:
...
- `src/app/api/global-fetcher.ts` — Centralized data fetching utility
- `src/app/api/*/` — Feature-specific API routes and handlers

### Utilities & Configuration

- `src/hooks/use-mobile.ts` — Mobile breakpoint detection hook
- `src/lib/utils.ts` — Shared utility functions (cn, formatting, etc.)
- `src/utils/i18n.ts` — Internationalization setup
- `src/utils/languages/` — Language JSON files (ar.json, ch.json, en.json, fr.json)
- `src/app/css/pages/` — Page-specific CSS overrides
- `src/app/dashboard-globals.css` (dashboard; public site uses `src/app/globals.css`) — Global styles with Tailwind directives
...
1. **Primitive Components** (`src/components/ui/`) — Base UI + shadcn/ui wrappers with Tailwind styling
2. **Feature Components** (`src/components/dashboard/*/`) — Domain-specific components
3. **Page Components** (`src/app/(dashboard)/dashboard/*/page.tsx`) — Page-level layouts
4. **Context Providers** (`src/app/context/*/`) — State management containers

### State Management Pattern

- **Global Config**: `ConfigContext` in `src/app/context/config.ts`
...
- **Theme Config**: `src/app/context/config.ts`
...
- **Typography**: Font stack defined in `src/app/dashboard-globals.css`
- **Shadows**: Tailwind shadow utility classes
- **Border Radius**: Configurable in `src/app/context/config.ts` (default: 12px)
...
1. Create in appropriate `src/app/(dashboard)/dashboard/*/` directory
...
1. Create in `src/app/api/` with feature-specific subdirectories
2. Use `src/app/api/global-fetcher.ts` for client-side data fetching
...
1. Create directory in `src/app/(dashboard)/dashboard/apps/[feature]/`
2. Create context provider in `src/app/context/[feature]-context/`
3. Create components in `src/components/dashboard/apps/[feature]/`
4. Add API routes in `src/app/api/[feature]/`
5. Register in navigation/menu

### Implementing a New Feature with Context

1. Create context file: `src/app/context/[feature]-context/index.tsx`
...
1. Add translations to language JSON files in `src/utils/languages/`
...
2. Modify theme options in `src/app/context/config.ts`
...
import { useConfig } from "@/src/app/context/config";
...
**Template**: ShadcnSpace Dashboard Pro
