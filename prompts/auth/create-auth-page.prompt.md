...
- **Target Page File**: `{{PAGE_FILE}}` (e.g., `src/app/(dashboard)/auth/auth1/login/page.tsx`)
...
1. **Separation of Concerns**:
   - **Form Component**: Create in `src/app/(dashboard)/auth/authforms/` (e.g., `auth-login.tsx`).
   - **Validation Config**: Create/Update `src/app/(dashboard)/auth/authforms/validation-config.ts` to store all error messages.
   - **Page Route**: Create in `src/app/(dashboard)/auth/auth1/...` or `src/app/(dashboard)/auth/auth2/...`.
...
3. **UI & Styling Rules**:
   ...
   - Integrate `FullLogo` from `@/src/app/(dashboard)/dashboard/layout/shared/logo/full-logo`.
...
Add messages to `src/app/(dashboard)/auth/authforms/validation-config.ts`.
```typescript
