...
1. **Explore Dashboard Structure**: Check `src/app/(dashboard)/dashboard/dashboards/` to see all available dashboard types (analytics, ecommerce, crm, saas, etc.).
...
3. **Component Reuse**: Identify reusable components from `src/components/dashboard/dashboards/` that can be adapted for the new dashboard.
...
1. **Research Phase**: Examine existing dashboards in `src/app/(dashboard)/dashboard/dashboards/` and `src/components/dashboard/dashboards/` to understand fluid layout patterns and UI component standards.
2. **Directory**: `src/components/dashboard/dashboards/{{DASHBOARD_NAME_LOWER}}/`
...
5. **Page**: Construct the assembled layout in `src/app/(dashboard)/dashboard/dashboards/{{DASHBOARD_NAME_LOWER}}/page.tsx`.
