...
## Input Variables
- **Target Component File**: `{{COMPONENT_FILE}}` (e.g., `src/components/dashboard/form-components/my-domain/custom-form.tsx`)
- **Target Page File**: `{{PAGE_FILE}}` (e.g., `src/app/(dashboard)/dashboard/forms/my-domain/page.tsx`)
...
1. **Separation of Concerns (IMPORTANT)**: 
   - The reusable **Form UI Component** MUST be created inside `src/components/dashboard/form-components/...`.
   - The **Page Route** that renders the form MUST be created inside `src/app/(dashboard)/dashboard/forms/...`.
...
<form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
  <div>
    ...
    {errors.fieldName && <span className="text-destructive text-sm">{errors.fieldName.message}</span>}
  </div>
</form>
```
...
**A. Vertical Sidebar (`src/app/(dashboard)/dashboard/layout/vertical/sidebar/sidebaritems.ts`)**
...
**B. Horizontal Menu (`src/app/(dashboard)/dashboard/layout/horizontal/menudata.ts`)**
