<!-- DESCRIPTION: Completely Remove a Language from the ShadcnSpace Dashboard Pro
...
**Target File:** `src/app/(dashboard)/dashboard/layout/shared/header/language.tsx`
...
1.  **Default Language:** Check `utils/i18n.ts` and `src/app/context/config.ts`. If the removed language was set as `lng` (default) or `fallbackLng`, update them to another existing code (usually `en`).
