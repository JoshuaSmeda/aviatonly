...
import {
  ChatHistoryItem,
  ChatSession,
} from "@/app/(dashboard-layout)/types/apps/ai-chat";
...
const ChatAiListing = ({ setShowChatWindow }: ChatSidebarProps) => {
  ...
  return (
    <>
      <div className="border-e p-0 w-full h-full flex flex-col">
          ...
          {unifiedChats.length === 0 ? (
            ...
            unifiedChats.map((chat, index) => (
              <div key={chat.id} className="relative px-5 group">
                <div
                  className={`cursor-pointer py-4 px-4 gap-0 flex justify-between  rounded-xl  group-hover:bg-primary/5 `}
                  onClick={(e) => {
                    e.stopPropagation();
                    setChatList([]);
                  ...
                  <div className="flex items-center gap-3 w-full">
                    <div className="group-hover:w-60 overflow-hidden">
                      ...
                      <p className="text-xs  text-muted-foreground
                       font-semibold truncate">{chat?.preview}</p>
                    </div>
                  </div>
                </div>
                <div className="absolute top-5 right-6 invisible group-hover:visible">
                  <DropdownMenu>
                    <DropdownMenuTrigger>
                      <Button className={
                        "bg-primary/5 text-primary hover:bg-primary/5 hover:text-primary cursor-pointer"
                      }>
