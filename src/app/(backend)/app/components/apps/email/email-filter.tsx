...
const EmailFilter = () => {
  ...
  return (
    <>
      <div className="border-e max-w-60 h-full w-full p-0">
        <div className="p-6">
          <EmailCompose />
        </div>
          ...
          <ul className="mb-20 flex flex-col gap-0.5 p-6 pt-0">
            {filterData.map((item) => {
              ...
              return (
                <li
                  key={item.id}
                  className={`py-2.5 gap-2 px-4 rounded-md cursor-pointer capitalize flex text-muted-foreground ${filter === item.name
                    ? "text-primary bg-primary/5"
                    : " rounded-md  hover:bg-primary/5 hover:text-primary"
                    }`}
