...
import { ArrowRight, CalendarDays, Handbag, LucideIcon, ShoppingBag } from "lucide-react";
    ...
    <div className="grid grid-cols-12 gap-6 h-full">
      <div className="col-span-12 xl:col-span-6 h-full">
        <Card className="p-0 ring-0 border rounded-xl relative h-full">
          <CardContent className="p-0 h-full">
            <div className="ps-6 py-4 flex flex-col gap-9 justify-between h-full">
              <div>
            ...
            {/* image */}
            <img
              src="/images/backgrounds/ecommerce-img.svg"
              alt="user-img"
              width={211}
              height={168}
              className="absolute -bottom-4 end-2 hidden sm:block dark:hidden"
            />
            <img
              src="/images/backgrounds/ecommerce-img-dark.svg"
              alt="user-img"
              width={211}
              height={168}
              className="absolute -bottom-4 end-2 hidden sm:block"
            />
          </CardContent>
        </Card>
      </div>
      {secondaryStats.map((stat, index) => (
        <div key={index} className="col-span-12 sm:col-span-6 xl:col-span-3">
          <Card className="py-6 ring-0 border rounded-xl">
            <CardContent className="px-6 flex items-start justify-between">
              <div className="flex flex-col gap-5 justify-between">
                ...
                <Button
                  variant={"outline"}
                  className={
                    "flex items-center gap-1.5 w-fit rounded-lg cursor-pointer shadow-xs h-9"
                  }
                >
