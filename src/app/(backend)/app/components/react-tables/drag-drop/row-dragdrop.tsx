...
import TitleIconCard from "@/app/components/shared/titleicon-card";
import {
  DndContext,
  DragEndEvent,
  MouseSensor,
...
const basicTableData5: RowdragdropTableType[] = [
  {
    ...
    users: "4300 Users",
    courses: [
      { status: "Angular", statuscolor: "destructive" },
      { status: "PHP", statuscolor: "chart-4" },
    ],
  },
  {
    id: "2",
    ...
    cell: (info) => (
      <div className="flex gap-2">
        {info.getValue().map((course, index) => (
          <Badge
            key={index}
            className={`bg-${course.statuscolor}  capitalize`}
          >
            {course.status}
          </Badge>
        ))}
