...
const columns = [
  columnHelper.accessor("actions", {
    cell: ({ row }) => (
      <span className="btn-circle-hover" onClick={() => row.toggleExpanded()}>
        {row.getIsExpanded() ? (
    ...
    cell: (info) => (
      <div className="flex gap-2">
        {info.getValue().map((course, index) => (
          <Badge
            key={index}
            className={`capitalize bg-${course.statuscolor} text-white`}
          >
            {course.status}
          </Badge>
        ))}
                    ...
                    {row.getIsExpanded() && (
                      <tr>
                        <td colSpan={columns.length} className="p-4">
                          <div className="flex flex-col gap-2 p-6 border border-border  rounded-md bg-muted dark:bg-muted/50">
                            ...
                            <p>
                              ...
                              {row.original.courses.map((course, index) => (
                                <span
                                  key={index}
                                  className={`badge bg-${course.statuscolor} text-white mr-2 rounded-full py-1 px-2 text-xs`}
                                >
                                  {course.status}
                                </span>
                              ))}
