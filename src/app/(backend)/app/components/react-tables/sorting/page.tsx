...
const columns = [
  ...
  columnHelper.accessor("status", {
    cell: (info) => {
      const StatusIcon = info.row.original.statusicon;
      return (
        <Badge
          className={`capitalize inline-flex items-center gap-1 bg-${info.row.original.statuscolor}`}
        >
                ...
                {table.getHeaderGroups().map((headerGroup) => (
                  <tr key={headerGroup.id}>
                    {headerGroup.headers.map((header) => (
                      <th
                        key={header.id}
                        className="text-base  font-semibold py-3 text-left border-b border-border px-4 cursor-pointer"
