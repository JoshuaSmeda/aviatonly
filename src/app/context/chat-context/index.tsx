...
import {
  ChatsType,
  MessageType,
} from "@/app/(dashboard)/dashboard/types/apps/chat";
...
export interface ChatContextProps {
  ...
  sendMessage: (chatId: number | string, message: string) => void;
...
export const ChatProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const sendMessage = async (chatId: number | string, message: string) => {
    try {
      let { data } = await mutate(
        postFetcher("/api/chat", { chatId, message })
      );
      let chat = data.find((chat: ChatsType) => chat.id === chatId);
