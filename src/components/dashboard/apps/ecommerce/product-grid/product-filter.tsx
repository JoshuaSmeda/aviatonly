...
import { ProductFiterType } from "@/app/(dashboard)/dashboard/types/apps/ecommerce";
...
const ProductFilter = () => {
  ...
  return (
    <div className="w-full border-e border-border">
      {/* Category */}
      <ul className="my-4 mt-0 pt-4 flex flex-col gap-1">
        {filterCategory.map((filter) =>
          filter.filterbyTitle ? (
            ...
            <li
              key={filter.id}
              className={`flex items-center py-3 gap-2 px-4 hover:bg-primary/5  hover:text-primary rounded-md cursor-pointer mx-6 ${selectedCategory === filter.sort
      ...
      <ul className="mt-0 px-6 flex flex-col gap-1">
        ...
        {filterbySort.map((filter) => (
          <li
            key={filter.id}
            className={`flex w-full items-center py-3 gap-2 px-4 hover:bg-primary/5  hover:text-primary rounded-md cursor-pointer  ${sortBy === filter.value ? "text-primary bg-primary/5   " : "text-muted-foreground"
              }`}
        ...
        >
          {genders.map((gen) => {
            return (
              <div
                key={`gender-${gen.id}`}
                className="group py-3 gap-2 hover:bg-primary/5 hover:text-primary px-4 rounded-md cursor-pointer text-muted-foreground"
              >
      ...
      <div className="mt-0 px-6">
        ...
        >
          {prices.map((price) => (
            <div
              key={price.id}
              className=" group flex py-3  px-4 gap-2  items-center rounded-md hover:bg-primary/5  cursor-pointer text-muted-foreground"
            >
              <RadioGroupItem
                value={price.radioid}
      ...
      <div className="mt-0 px-6 pb-6">
        ...
        <div className="flex flex-row flex-wrap gap-2 mb-7">
          {filterbyColors.map((theme, idx) => (
            ...
            >
              {selectedColor === theme && (
                <Icon icon="material-symbols-light:check-rounded" />
              )}
            </Label>
          ))}
        </div>
